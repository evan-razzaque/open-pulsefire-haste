...
static void select_profile(MouseProfileButton *self, GtkButton *button) {
    ...
    // gtk_widget_set_sensitive(GTK_WIDGET(self->button_delete), false)
}

static const char* get_name(MouseProfileButton *self) {
    return gtk_button_get_label(self->button_name);
}

static void set_name(MouseProfileButton *self, const char *name) {
...
static void edit_profile_name(MouseProfileButton *self, GtkButton *button) {
    ...
    if (self->is_editing_name) {
        gtk_editable_set_text(GTK_EDITABLE(self->editable_label_name), get_name(self));
...
static void change_name(MouseProfileButton *self, GParamSpec *is_editing_name, GtkEditableLabel *editable_label_name) {
    self->is_editing_name = gtk_editable_label_get_editing(self->editable_label_name);

    if (!self->is_editing_name) {
        toggle_name_editing(self, false);
        set_name(self, gtk_editable_get_text(GTK_EDITABLE(self->editable_label_name)));
    }
...
static void mouse_profile_button_class_init(MouseProfileButtonClass *klass) {
    ...
    gtk_widget_class_bind_template_callback(widget_class, change_name);
}
