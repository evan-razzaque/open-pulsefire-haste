...
int save_settings(hid_device *dev, color_options *color) {
	uint8_t d1[PACKET_SIZE] = {0xda, 0x01, 0x00, 0x3c, color->red, color->green, color->blue};
	mouse_write(dev, d1);

	for (int i = 0; i < 5; i++) {
		uint8_t d2[PACKET_SIZE] = {0xda, 0x01, i + 1, 0x3c};
		mouse_write(dev, d2); 
	}

	uint8_t d3[PACKET_SIZE] = {0xd9, 0x00, 0x00, 0x03, 0x55, 0x01, 0x23};
	mouse_write(dev, d3);

	uint8_t d4[PACKET_SIZE] = {0xdb, 0x55};
	mouse_write(dev, d4);

	uint8_t d5[PACKET_SIZE] = {SAVE_SETTINGS, 0xff};
