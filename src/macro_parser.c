 ...
 */
static void parse_mouse_event(macro_parser_data *p, generic_macro_event *generic_event, MACRO_EVENT_TYPE event_type) {
    p->event_count++;
...
int parse_macro(mouse_macro *macro, macro_event *events, byte *modifier_map) {
    ...
    for (int i = 0; i < macro->generic_event_count; i++) {
        ...
        case MOUSE_UP: 
            ...
            break;
        default:
            printf("%.4x is not a valid action_event_type\n", action_event_type);
            exit(-1);
            break;
        }
    }
