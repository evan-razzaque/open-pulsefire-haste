...
static void close_keyboard_actions_window(GtkButton *self, app_data *data) {
	gtk_window_close(data->widgets->test_window);
}

static void apply_keyboard_action_binding(GtkButton *self, app_data *data) {
...
void app_config_buttons_init(GtkBuilder *builder, app_data *data) {
	...
	g_signal_connect(data->widgets->test_window, "close-request", G_CALLBACK(clear_key_pressed_label), data->widgets->label_pressed_key);
	...
	widget_add_event(builder, "buttonKeybindConfirm", "clicked", apply_keyboard_action_binding, data);
